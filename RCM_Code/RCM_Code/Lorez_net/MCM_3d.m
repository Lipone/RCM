...
for coupling_strength = coupling_strengths
    for connection_prob = connection_probs
        numNodes = num_nodes;
        NCMindex=zeros(nets,numNodes,numNodes);
        GCindex=zeros(nets,numNodes,numNodes);
        TEindex=zeros(nets,numNodes,numNodes);
        CCMindex=zeros(nets,numNodes,numNodes);
        connectivityMatrix = cell(nets, 1);  % 使用 cell 数组来存储每次运行的邻接矩阵
        parfor run= 1:nets
            % 生成随机的邻接矩阵
            ...
            for i=1:numNodes
                for j=1:numNodes
                    ...
                    GCindex(run,j,i)=GCmy(data1(:,1:7000),data2(:,1:7000),p);%GC
                    TEindex(run,j,i)=TEmy(data1(:,1:7000),data2(:,1:7000),p,k,Thei);%TE
...
%         avg_auc_NCM = plot_avg_roc_with_std(predict_list, ground_truth_list, 'b', 'My Classifiers');
        ...
        save(sprintf("3D_CCMindex_coupling_%d_prob_%.1f.mat", coupling_strength, connection_prob), ...
...
% 
% load("NEW_output_coupling_1.000000e-01_prob_0.3.mat");
filename = '3D_CCMindex_coupling_1_prob_0.1';  % 你的 .mat 文件名
