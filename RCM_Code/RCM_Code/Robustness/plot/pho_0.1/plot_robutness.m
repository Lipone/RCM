...
% 添加图例
h = legend({'RCM','' , 'MCM'}, 'Location', 'northeast', 'TextColor', 'k', 'Box', 'off','FontWeight', 'bold');  
% h.Position = [0.6, 0.2, 0.2, 0.1];  % 你可以微调这四个参数
...
filename = 'RCM_pho0.1_InnerNoise.mat';  % 你的 .mat 文件名
% 获取不带扩展名的文件名
[~, name, ~] = fileparts(filename);
load(filename);
...
% xlabel('$\mathbf{\beta_{xy}}$', 'Interpreter', 'latex', 'FontSize', 14, 'FontWeight', 'bold');
% 添加图例
h = legend({'RCM [with causation]','' , 'RCM [without causation]'}, 'Location', 'southeast', 'TextColor', 'k', 'Box', 'off','FontWeight', 'bold');  
h.Position = [0.52, 0.2, 0.2, 0.1];  % 你可以微调这四个参数
legend('show');  % 隐藏顶部和右侧坐标轴
ax.Box = 'off';
ax.XAxis.Visible = 'on';
ax.YAxis.Visible = 'on';
...
%% plot outter noise
%% plot outer noise
filename = 'RCM_pho0.1_outterNoise.mat';  % 你的 .mat 文件名
% 获取不带扩展名的文件名
[~, name, ~] = fileparts(filename);
load(filename);
...
% 选择 x 范围为 0.5 到 1 之间的数据
x_select = x(x >= 0.5 & x <= 1);
yu_select = yu(x >= 0.5 & x <= 1);
yl_select = yl(x >= 0.5 & x <= 1);
MEAN_select = MEAN(x >= 0.5 & x <= 1);

x_interp = linspace(min(x_select), max(x_select), 100);
yu_interp = interp1(x_select, yu_select, x_interp, 'spline');
yl_interp = interp1(x_select, yl_select, x_interp, 'spline');
color_mean_ones = color_all(6, :);
color_between_ones = CList(2, :);

plot(x_select, MEAN_select, 'Linewidth', 2, 'DisplayName', 'CCM[X→Y]', 'color', color_mean_ones);
hold on;
fill([x_select, fliplr(x_select)], [yu_select, fliplr(yl_select)], color_between_ones, 'linestyle', 'none', 'FaceAlpha', 0.3);
...
plot(x_select, MEAN_select, 'Linewidth', 2, 'DisplayName', 'CCM[Y→X]', 'color', color_mean_zeros);
fill([x_select, fliplr(x_select)], [yu_select, fliplr(yl_select)], color_between_zeros, 'linestyle', 'none', 'FaceAlpha', 0.3);
...
% 选择 y 范围为 x 范围 0.5 到 1 之间的数据
y_select = y(:, x >= 0.5 & x <= 1);

% 使用循环绘制每列数据的散点图
for i = 1:size(y_select, 2)  % 遍历每一列
    scatter(x_select, y_select(i, :), 10, CList(1, :), 'filled');  % 将第 i 列的数据绘制成散点
end
hold on;

% 展开NCM_values_for_ones中的数据，假设每个单元都是相同长度的数组
y = []; % 用来存储所有的 y 值

% 将NCM_values_for_ones中每个数组的数据存储到 y 中
for i = 1:length(all_ones)
    y = [y, all_ones{i}]; % 将每个单元的内容添加到y数组
end
...
% 使用循环绘制每列数据的散点图
for i = 1:size(y_select, 2)  % 遍历每一列
    scatter(x_select, y_select(i, :), 10, CList(2, :), 'filled');  % 将第 i 列的数据绘制成散点
end
hold on;
...
%% 画长度
filename = 'MCM_Length.mat';  % 你的 .mat 文件名
% 获取不带扩展名的文件名
[~, name, ~] = fileparts(filename);
load(filename);
...
% xlabel('$\mathbf{\beta_{xy}}$', 'Interpreter', 'latex', 'FontSize', 14, 'FontWeight', 'bold');
% 添加图例
legend({'MCM [with causation]','' , 'MCM [without causation]'}, 'Location', 'northeast', 'TextColor', 'k', 'Box', 'off','FontWeight', 'bold');  
legend('show');  
% 隐藏顶部和右侧坐标轴
ax.Box = 'off';
ax.XAxis.Visible = 'on';
...
exportgraphics(fig, [name, '.jpg'], 'Resolution', 1000);
%% plot emb
filename = 'Diffrent_Emb_effect.mat';  % 你的 .mat 文件名
% 获取不带扩展名的文件名
[~, name, ~] = fileparts(filename);
load(filename);
